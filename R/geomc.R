...
#' the log un-normalized pdf or pmf. The base density can be specified along with its mean, covariance matrix, and a function
#' for sampling from it. Gaussian densities can be specified by mean and variance only, although it is also preferred to supply density
#' and sampling functions. If either or both of the mean and variance arguments and any of the density and sampling functions is
...
#' can be specified along with their means, covariance matrices, and a function for sampling from the density.
#' Gaussian densities can be specified by mean and variance only, although it is also preferred to supply density and sampling
#' functions. If either or both of the mean and variance
...
#' @param log.target is the logarithm of the (un-normalized) target density function, needs to be written as a function of the current value x
...
#' @param ind is False if either f (the base density) or g (the approximate target density) depends on x. Default is False.
#' @param gaus is True if both f and g are normal distributions. Default is True.
...
#' imp \eqn{[3]} (samp.base) is True if samples from f is used, otherwise samples from g is used. Default is False.
#' @param a is the probability vector for the mixture proposal density. Default is the uniform distribution.
#' @param mean.base is the mean of the base density f, needs to be written as a function of current value x.
#' @param var.base is the covariance matrix of the base density f, needs to be written as a function of current value x.
#' @param dens.base is the density function of the base density f, needs to be written as a function (y,x) (in this order) of the proposed value y and the current value x, although it may not depend on x.
#' @param samp.base is the function to draw from the base density f, needs to be written as a function of current value x.
#' @param mean.ap.tar is the vector of means of the densities g_i(y|x), i=1,dots,k. It needs to be written as a function of current value x. It must have the same dimension as k times the length of initial.
#' @param var.ap.tar is the matrix of covariance matrices of the densities g_i(y|x), i=1,dots,k formed by column concatenation. It needs to be written as a function of current value x. It must have the same dimension as the length of initial by k times the length of initial.
#' @param dens.ap.tar is the vector of densities g_i(y|x), i=1,dots,k. It needs to be written as a function (y,x) (in this order) of the proposed value y and the current value x, although it may not depend on x.
#' @param samp.ap.tar is the function to draw from the densities g_i(y|x), i=1,dots,k. It needs to be written as a function of (current value x, the indicator of mixing component kk). It must return a vector of the length of that of the initial.
#' @details
